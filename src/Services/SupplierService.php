...
use WebWMS\Entity\Supplier as Suppliers;
...
class SupplierService
{
    ...
    {
        $customers = $this->doctrine->getRepository(Suppliers::class)->findAll();

        if (!$customers) {
            throw $this->createNotFoundException('Keine Lieferanten gefunden');
        }
    ...
    {
        //dd($request->request->all()['supplier']);
    ...
    public function getLastSupplier(): array
    {
        ...
        return $customerOrderRepository->findBy([], ['supplier_nr' => 'DESC'], 1, 0);
    }
