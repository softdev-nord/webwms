...
final readonly class DbalInventoryRepository implements InventoryRepository
{
    ...
    public function savePickList(PickList $pickList): void
    {
        $this->connection->transactional(function (Connection $connection) use ($pickList): void {
            if ($connection->fetchOne(
                "SELECT 1 FROM wms_outbound_order o, wms_user_account u WHERE o.id = :orderId AND o.tenant_id = :tenantId AND o.status = 'released' AND u.id = :userId AND u.tenant_id = :tenantId FOR UPDATE",
                ['orderId' => $pickList->outboundOrderId()->value(), 'userId' => $pickList->createdBy()->value(), 'tenantId' => $pickList->tenantId()->value()],
            ) === false) {
            ...
            foreach ($pickList->allocationIds() as $sequence => $allocationId) {
                if ($connection->fetchOne(
                    "SELECT 1 FROM wms_stock_allocation a INNER JOIN wms_stock_reservation r ON r.id = a.reservation_id INNER JOIN wms_outbound_order_item i ON i.reservation_id = r.id WHERE a.id = :id AND a.tenant_id = :tenantId AND a.status = 'active' AND i.outbound_order_id = :orderId FOR UPDATE",
                    ['id' => $allocationId->value(), 'tenantId' => $pickList->tenantId()->value(), 'orderId' => $pickList->outboundOrderId()->value()],
                ) === false) {
