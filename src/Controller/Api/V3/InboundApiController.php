...
final class InboundApiController extends AbstractController
{
    ...
    public function create(Request $request): JsonResponse
    {
        ...
        if (!is_array($items)) { throw new \InvalidArgumentException('Field "items" must be an array.'); }
        ...
        foreach ($items as $item) {
            if (!is_array($item)) { throw new \InvalidArgumentException('Each receipt item must be an object.'); }
            $normalized[] = [
    ...
    {
        $user = $this->getUser();
        if (!$user instanceof TenantPermissionUser) { throw $this->createAccessDeniedException('An authenticated tenant user is required.'); }
        return $user;
    }
    ...
    private function string(array $payload, string $field): string
    {
        ...
        if (!is_string($value) || trim($value) === '') { throw new \InvalidArgumentException(sprintf('Field "%s" must be a non-empty string.', $field)); }
        return trim($value);
    }
    /** @param array<string, mixed> $payload */
    ...
    {
        $value = $payload[$field] ?? null;
        if (!is_int($value)) { throw new \InvalidArgumentException(sprintf('Field "%s" must be an integer.', $field)); }
        return $value;
    }
}
